...
**all-in-dd**는 *all in direct design*, all-in-bb의 오마주다. 코딩 에이전트와 비주얼
작업을 하기 위한 디자인 시스템 골격으로, 브랜드 하나의 값을 DTCG 토큰 JSON 한 곳에
두고 빌드 한 번으로 네 매체에 뿌린다 — 웹, 슬라이드 덱, 카드뉴스, 인쇄.
...
값은 `tokens/`에, 성격은 `DESIGN.md`에, 규칙은 `ENGINE.md`에 산다. 셋이 서로
새기 시작하면 시스템이 죽어가는 중인 거다.
